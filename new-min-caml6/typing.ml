...
let rec print_syntax_t e num =
  ...
  | Add (e1,e2) -> print_newline(); print_blank num; Printf.printf "ADD "; print_syntax_t e1 (num+1); print_syntax_t e2 (num+1);
  | Sub (e1,e2) -> print_newline(); print_blank num; Printf.printf "SUB "; print_syntax_t e1 (num+1); print_syntax_t e2 (num+1);
  | FNeg e1 -> print_newline(); print_blank num; Printf.printf "FNEG "; print_syntax_t e1 num;
  | FAdd (e1,e2) -> print_newline(); print_blank num; Printf.printf "FADD "; print_syntax_t e1 (num+1); print_syntax_t e2 (num+1);
  | FSub (e1,e2) -> print_newline(); print_blank num; Printf.printf "FSUB "; print_syntax_t e1 (num+1); print_syntax_t e2 (num+1);
  | FMul (e1,e2) -> print_newline(); print_blank num; Printf.printf "FMUL "; print_syntax_t e1 (num+1); print_syntax_t e2 (num+1);
  | FDiv (e1,e2) -> print_newline(); print_blank num; Printf.printf "FDIV "; print_syntax_t e1 (num+1); print_syntax_t e2 (num+1);
  ...
  | Let((x, t), e1, e2) -> print_newline(); print_blank num; print_string x; print_syntax_t e1 (num+1); print_syntax_t e2 (num+1);
...
let rec convert exp =
  ...
  | LetRec({ name = (x, t); args = yts; body = e1 }, e2) ->
      ...
      | first :: rest ->
        ...
        LetRec({name = (addtyp x); args = [first];
                body = convert (LetRec({name = (addtyp funname); args = rest; body = e1},Var(funname)))}, convert e2))
  | App (Var(e), es) -> 
    (match es with
    | first :: [] ->
        App (Var(e),[(convert first)])
    | first :: rest ->
        convert(App(App(Var(e),[(convert first)]),rest))
    )
